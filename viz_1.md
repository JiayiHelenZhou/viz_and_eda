...
Create the first scatterplot ever\!
...
start with the same one and then make it fancy:
...
``` r
# alpha is the transparency, and it can be put in the geom_pont() too. size of the point and the smooth are also adjustable.
#facet_grid(row, col)
```
...
How many geoms have to exist? you can have whatever geom you want.
...
``` r
# will smooth out the histogram, and use adjust to give more detail.
```
...
``` r
# more informative than box plot, since they show the density.
```
